...
public class branchandbound {
    ...
    if (branching.father != null) { // stop before root node
      if (branching.branchValue == 0) { // forbid this edge (in this direction)
        ...
        if (branching.branchFrom != 0) {
          ...
          for (i++; i < userParam.nbclients + 2; i++)     // 这里的表述，是有意为之？  i++ 开始，是一种什么表达？ i 是上一个循环的结尾处的值
            userParam.dist[branching.branchFrom][i] = userParam.verybig;
        }
  ...
  public boolean BBnode(paramsVRP userParam, ArrayList<route> routes,
    ...
    try {
      ...
      CGobj = CG.computeColGen(userParam, routes);    // 这里返回的，就是列生成算法求解后的值嘛？ 是的！
      // feasible ? Does a solution exist?
      if ((CGobj > 2 * userParam.maxlength) || (CGobj < -1e-6)) { 
        // can only be true when the routes in the solution include
        // forbidden edges (can happen when the BB set branching values)
      ...
      if (branching.lowestValue > upperbound) {     // 如果解出来的值，比已有的解 花费更高，则废弃这一路线。
        CG = null;
        ...
        * */
          ...
          for (i = 0; i < userParam.nbclients + 2; i++) {
            for (j = 0; j < userParam.nbclients + 2; j++) {
              coef = userParam.edges[i][j];  //  userParam.edges : 在分枝和定界过程中每条边的权重
              if ((coef > 1e-6)
          ...
          if (feasible) {
            if (branching.lowestValue < upperbound) { // new incumbant feasible solution! // 得到新的可行解
              ...
              System.out.println("FEAS | Lower bound: " + lowerbound
                  ...
                  + depth + " | Local CG cost: " + CGobj + " | " + routes.size()
                  + " routes");
            return feasible;        // 递归时，返回值为 True 。
          } else {      //  feasible 为 false 时的情况
            ...
            * 当不可解时，进一步往下分支，寻求解。
            * */
            // first branch -> set edges[bestEdge1][bestEdge2]=0： 分支 1
            ...
            // second branch -> set edges[bestEdge1][bestEdge2]=1 : 分支2
            // record the branching information in a tree list
            ...
            // second branching=>need to reinitialize the dist matrix： 第二个分支=>需要重新初始化dist矩阵
            for (i = 0; i < userParam.nbclients + 2; i++)
              ...
              if (path.size() > 3) { // we must keep trivial routes
                  ...
                  for (i = 1; accept && (i < path.size()); i++) {
                    ...
                    if (userParam.dist[prevcity][city] >= userParam.verybig - 1E-6)
                      accept = false;   // 只要有一个点走不通（值太大），这条路也走不通
                    prevcity = city;
                  }
                }
              if (accept)
