...
public class GeneticAlgorithm {
    ...
    SpeciesIndividual run(SpeciesPopulation list){
        ...
        for(int i=1; i<TSPData.DEVELOP_NUM;i++){    //这里是进化迭代了 1000 次
            //选择
            select(list);
...
//			}
    ...
    void select(SpeciesPopulation list){
        ...
        while(point != null){
            if(talentDis > point.distance){
                talentDis = point.distance;     //距离最小，适应度就最大
                talentSpecies = point;
            }
            point = point.next;
        }

        // 将最大适应度物种复制talentNum个,总数的 1/4
        ...
        for(int i=1; i<=roundNum; i++){
            ...
            while(oldPoint != null && oldPoint != talentSpecies){       //排除 空 和 最优节点（已选）
                if(rate <= oldPoint.rate){
                    ...
                    rate = rate - oldPoint.rate;        //减少rate，避免选不到的情况
                }
    ...
    void crossover(SpeciesPopulation list) {
        ...
        if (rate > TSPData.pcl && rate < TSPData.pch) {
            ...
            int find = rand.nextInt(list.speciesNum);   // 种群 个体数总数中 选了个随机数
            while (point != null && find != 0)          //寻找表尾结点?  应该是point 向前走了find步，同时find是随机变量
            {
                point = point.next;
                find--;
            }
            ...
            if(point.next != null)
            {
                ...
                for(int i=begin;i<TSPData.CITY_NUM;i++)
                {
                    ...
                    //两个基因互换
                    // 从begin 开始，point 和 point.next 的每个相同位置 基因互换
                    String tmp;
                    tmp=point.genes[i];
                    ...
                    //消去互换后重复的那个基因
                    point.genes[fir]=point.next.genes[i];       //新的point.next 的i位置的值
                    point.next.genes[sec]=point.genes[i];       // 妙哉！ 这里的处理很奇妙，保证没有重复！
    ...
    SpeciesIndividual getBest(SpeciesPopulation list)
    {
        ...
        {
            if(distance > point.distance)       //通过距离找到最优的个体
            {
                bestSpecies=point;
