"""
学习博文链接：https://mp.weixin.qq.com/s/2dB-XvcPNeDyJ4fXsW_8jA
"""
...
SE_LIST=[]              #可扫描节点集合
SE_LIST_M = set()       #记录有哪些点曾经进入过SE_LIST
...
for i in range(len(df_link)):
    ...
    g_adjacent_arc_list[df_link[i,1]].append(df_link[i,2])      # df_link[i,1] 可到达的点集合
SE_LIST=[g_origin]
...
while len(SE_LIST):
    ...
    for tail in adjacent_arc_list:
        if node_label_cost[tail-1]> node_label_cost[head-1]+Distance[head-1,tail-1]:
            ...
            if (g_node_status[tail-1]==0 and (tail not in SE_LIST_M)  ):
                SE_LIST.append(tail)    #将新节点添加到可扫描列表尾部,append方法实现从列表尾部添加元素
                SE_LIST_M.add(tail)     #记录有哪些点曾经进入过 SE_LIST
                g_node_status[tail-1]=1
            elif (g_node_status[tail-1]==0 and (tail in SE_LIST_M)  ):
                SE_LIST.inser(0,tail)   #将点添加到列表头，优先扫描
                g_node_status[tail-1]=1
...
file_data.to_csv('./agent_dllca.csv',index=False)
