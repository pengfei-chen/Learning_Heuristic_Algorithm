"""导入相关基础包，定义全局变量"""
"""
博文链接：https://mp.weixin.qq.com/s/WQUVOKGtzh8REm4vksAIUg
"""
...
g_link_list=[]          #网络弧集合
g_adjacent_arc_list={}  #节点邻接弧集合（从节点i发出的弧集合） 新增加
...
for i in range(len(df_node)):
    ...
    g_number_of_nodes+=1
    g_adjacent_arc_list[df_node[i,0]]=[]                #相邻啥？
    if df_node[i, 3] == 1:
...
for i in range(len(df_link)):
    ...
    g_adjacent_arc_list[df_link[i,1]].append(df_link[i,2])      # 点 df_link[i,1] 的相邻点，存进一个列表里
...
while len(SE_LIST):
    ...
    SE_LIST.remove(head)                        #将被抽取节点从集合中移除
    adajacent_arc_list = g_adjacent_arc_list[head]  #获取当前节点的邻接弧
    for tail in adajacent_arc_list:
        if node_label_cost[tail - 1] > node_label_cost[head-1] + Distance[head-1, tail-1]:
            ...
            if g_node_status[tail-1] == 0:
                SE_LIST.append(tail)            # tail 节点的最优路径改变了，其后的最优路径可能也会改变，因此将它加入SE_LIST
                g_node_status[tail-1] = 1       # 避免反复添加
...
file_data.to_csv('./agent_mlca.csv', index=False)
